...
from db import (
    fetch_subjects_to_display, fetch_year_groups_to_display, fetch_prompt,
    fetch_categories, fetch_comments2, insert_category, update_category_display,
    delete_category, insert_comment, update_comment, delete_comment, move_comment,
    update_prompt, fetch_existing_comments
)
...
class YearSelectDialog(wx.Dialog):
    ...
    def on_ok(self, event):
        ...
        for year_group_id, checkbox in self.checkboxes.items():
            ...
            update_category_display(year_group_id, display)
...
class ChangePromptDialog(wx.Dialog):
    ...
    def on_save(self, event):
        ...
        if subject_name and year_group_name:
            ...
            update_prompt(subject_id, year_group_id, prompt)
    ...
    def on_generate(self, event):
        ...
        if subject_name and year_group_name and reports:
            ...
            try:
                ...
                existing_comments = fetch_existing_comments(subject_id, year_group_id)
                existing_categories = {}
                ...
                delete_category(subject_id, year_group_id)

                for category_name, comments in merged_categories.items():
                    category_id = insert_category(category_name, subject_id, year_group_id)
                    for comment in comments:
                        insert_comment(comment, category_id)
...
class ExportDialog(wx.Dialog):
    ...
    def on_export(self, event):
        ...
        if subject_name and year_group_name:
            ...
            comments_data = fetch_existing_comments(subject_id, year_group_id)
...
class ImportDialog(wx.Dialog):
    ...
    def clear_categories(self, subject_id, year_group_id):
        ...
        if subject_name and year_group_name:
            ...
            if open_dialog.ShowModal() == wx.ID_OK:
                ...
                try:
                    ...
                    self.clear_categories(subject_id, year_group_id)
                    ...
                    for category_name, comment_text in data:
                        ...
                        insert_comment(comment_text, category_id)
...
class ManageCommentsDialog(wx.Dialog):
    ...
    def add_category(self, event):
        ...
        if name and subject_name and year_group_name:
            subject_id = self.subject_map[subject_name]
            year_group_id = self.year_group_map[year_group_name]
            insert_category(name, subject_id, year_group_id)
    ...
    def add_comment(self, event, category_id, text_ctrl):
        ...
        if text:
            insert_comment(text, category_id)
            text_ctrl.SetValue("")
            self.load_categories_and_comments(None)
        else:
